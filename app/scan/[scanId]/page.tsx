      ...
      <div className="grid md:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-black/5 bg-white p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-2.5">Detected items</h3>
            <ul className="space-y-1">
              {items.map((item) => (
                <li key={item.id}>
                  <button
                    onClick={() => setSelectedItem(item)}
                    className={`text-sm text-left w-full rounded-lg px-2.5 py-2 transition-colors flex items-center gap-2 ${
                  ...
                  >
                    ...
                    />
                    <span className="capitalize">{item.detected_label}</span>
                  </button>
                </li>
              ))}
